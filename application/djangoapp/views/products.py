...
def loadproduct(request):
    ...
    if request.method == 'POST' or request.method == 'GET':
        ...
        if prod != promo:
            ...
            p = json.loads(products)
            for c in p['produits']:
                record = ProductsEco(id = c['id'], codeProduit = c['codeProduit'], familleProduit = c['familleProduit'], descriptionProduit = c['descriptionProduit'], quantiteMin = c['quantiteMin'], packaging = c['packaging'], prix = c['prix'])
                record.save()
    ...
    if request.method == 'POST' or request.method == 'GET':
        ...
        if prod != promo:
            ...
            p = json.loads(products)
            for c in p['produits']:
                record = ProductsMag(id = c['id'], codeProduit = c['codeProduit'], familleProduit = c['familleProduit'], descriptionProduit = c['descriptionProduit'], quantiteMin = c['quantiteMin'], packaging = c['packaging'], prix = c['prix'])
                record.save()
